# API Details: Lấy danh sách nhân sự

**Version Control:**

| Version | Date       | Author         | Changes                        | Approved By | Status    |
| :--- | :--- | :---- | :----- | :---- | :----- |
| 1.0     | 2025-05-01 | Chiến Trần Văn | Tạo tài liệu API lấy danh sách nhân sự | -           | Draft     |

---

## 1. Mục tiêu  
Cung cấp API để lấy danh sách nhân sự trong hệ thống với các tùy chọn lọc, tìm kiếm và phân trang.

---

## 2. Overview

| Thuộc tính         | Giá trị                                      |
|--------------------|----------------------------------------------|
| **API Code**       | API-HRM-001                                  |
| **Tên API**        | Lấy danh sách nhân sự                        |
| **Mô tả**          | API cho phép lấy danh sách nhân sự với các bộ lọc và phân trang |
| **Module**         | Quản lý Nhân sự (HRM)                        |
| **Phương thức**    | `GET`                                        |
| **Endpoint**       | `/api/v1/employees`                          |
| **Quyền truy cập** | employee:read:all, employee:read:team, employee:read:basic, employee:read:own |

---

## 3. Parameters

### 3.1 Query Parameters

| Tên            | Kiểu dữ liệu         | Bắt buộc | Mô tả |
|----------------|----------------------|----------|-------|
| `keyword`      | String               | Không    | Tìm theo tên hoặc mã nhân viên |
| `teamId`       | Integer              | Không    | ID của team cần lọc |
| `position`     | String               | Không    | Vị trí công việc (Developer, Tester, BA, ...) |
| `status`       | String               | Không    | Trạng thái: `Allocated`, `Available`, `EndingSoon`, `OnLeave`, `Resigned` |
| `skills`       | Array of Integer     | Không    | Danh sách ID kỹ năng cần lọc |
| `minExperience`| Integer              | Không    | Số năm kinh nghiệm tối thiểu |
| `page`         | Integer              | Không    | Trang cần lấy (mặc định: `1`) |
| `size`         | Integer              | Không    | Số bản ghi mỗi trang (mặc định: `10`) |
| `sortBy`       | String               | Không    | Trường sắp xếp (mặc định: `name`) |
| `sortDir`      | String               | Không    | Hướng sắp xếp: `asc` hoặc `desc` (mặc định: `asc`) |
| `marginStatus` | String               | Không    | Lọc theo trạng thái margin: `Red`, `Yellow`, `Green` (chỉ dành cho Leader/課長) |

### 3.2 Header Parameters

| Tên             | Kiểu dữ liệu | Bắt buộc | Mô tả |
|-----------------|--------------|----------|-------|
| `Authorization` | String       | Có       | Định dạng: `Bearer {token}` |

### 3.3 Validate Rule
| Trường         | Điều kiện hợp lệ |
|----------------|------------------|
| `page`         | ≥ 1              |
| `size`         | 1 → 100          |
| `sortDir`      | `asc`, `desc`    |
| `status`       | Một trong: `Allocated`, `Available`, `EndingSoon`, `OnLeave`, `Resigned` |
| `marginStatus` | Một trong: `Red`, `Yellow`, `Green` |
---

## 4. Response

### 4.1 Success - 200 OK

```json
{
  "status": "success",
  "code": 200,
  "data": {
    "content": [
      {
        "id": 1,
        "employeeCode": "NV001",
        "name": "Nguyễn Văn A",
        "email": "a.nguyenvan@company.com",
        "position": "Developer",
        "team": {
          "id": 1,
          "name": "Team Alpha"
        },
        "status": "Allocated",
        "currentProject": "Project XYZ",
        "utilization": 100,
        "endDate": "2025-12-31",
        "skills": [
          {
            "id": 1,
            "name": "Java",
            "level": "Advanced",
            "years": 5
          },
          {
            "id": 2,
            "name": "Spring Boot",
            "level": "Intermediate",
            "years": 3
          }
        ],
        "marginStatus": "Green"
      },
      {
        "id": 2,
        "employeeCode": "NV002",
        "name": "Trần Thị B",
        "email": "b.tranthi@company.com",
        "position": "Tester",
        "team": {
          "id": 1,
          "name": "Team Alpha"
        },
        "status": "Available",
        "currentProject": null,
        "utilization": 0,
        "endDate": null,
        "skills": [
          {
            "id": 5,
            "name": "Selenium",
            "level": "Advanced",
            "years": 4
          },
          {
            "id": 6,
            "name": "Cucumber",
            "level": "Intermediate",
            "years": 2
          }
        ],
        "marginStatus": "Red"
      }
    ],
    "pageable": {
      "pageNumber": 1,
      "pageSize": 10,
      "totalPages": 5,
      "totalElements": 45,
      "sort": "name,asc"
    }
  }
}
```

### 4.2 Error Responses

#### 400 Bad Request
```json
{
  "status": "error",
  "code": "E2000",
  "message": "Tham số không hợp lệ",
  "errors": [
    {
      "field": "size",
      "message": "Kích thước trang phải nhỏ hơn hoặc bằng 100"
    }
  ]
}
```

#### 401 Unauthorized
```json
{
  "status": "error",
  "code": "E1000",
  "message": "Token không hợp lệ hoặc đã hết hạn"
}
```

#### 403 Forbidden
```json
{
  "status": "error",
  "code": "E1002",
  "message": "Không có quyền truy cập chức năng này"
}
``` 